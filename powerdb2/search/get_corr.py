...
def get_corr(ts1, ts2, period):
		...
		'''
		print "Timeseries1 ",ts1,len(ts1)
		print "Timeseries2 ",ts2,len(ts2)
...
#		 '''
#		 path1 = "/Users/hdz_1989/Documents/Dropbox/SDB/KETI/415/co2.csv"
#		 path2 = "/Users/hdz_1989/Documents/Dropbox/SDB/KETI/417/temperature.csv"
#		 rd1 = [float(i.strip('\n').split(',')[1].strip()) for i in open(path1,'r').readlines()]
#		 rd2 = [float(i.strip('\n').split(',')[1].strip()) for i in open(path2,'r').readlines()]
#		 # adujst to data to an array of even length
#		 ts1 = np.asarray(rd1)
#		 ts2 = np.asarray(rd2)
#		 print get_corr(ts1, ts2, 30)
